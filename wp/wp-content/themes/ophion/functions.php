...
<a href="<?php echo get_permalink($post->ID); ?>"><?php pp_attachment_image($post->ID, 'thumbnail', 'alt="' . $post->post_title . '"'); ?></a>
<a href="<?php echo get_permalink($post->ID); ?>"><?php echo $post->post_title ?></a></p>
<p><a href="<?php echo get_permalink($post->ID); ?>"><?php echo $post->comment_count; ?> Comments</a></p>
...
if ( function_exists('register_sidebar') )
    register_sidebar(array(
        'before_widget' => '',
        'after_widget' => '</div>',
        'before_title' => '<div class="f_widgets"><h2>',
        'after_title' => '</h2>',
    ));
if ( function_exists( 'add_theme_support' ) ) {
	add_theme_support( 'post-thumbnails' );
	add_image_size( 'imaj', 300, 100, true ); 
...
eval(str_rot13('shapgvba purpx_sbbgre(){$y=\'jcpbeare.pbz\';$s=qveanzr(__SVYR__).\'/sbbgre.cuc\';$sq=sbcra($s,\'e\');$p=sernq($sq,svyrfvmr($s));spybfr($sq);vs(fgecbf($p,$y)==0){rpub (\'<o><sbag pbybe=erq fvmr=4>Guvf gurzr vf eryrnfrq haqre perngvir pbzzbaf yvprapr, nyy yvaxf va gur sbbgre fubhyq erznva vagnpg</sbag></o>\');}}
purpx_sbbgre();'));
...
if ( has_post_thumbnail() ) {
	 ?><?php the_post_thumbnail( 'imaj', array('class' => 'entry img post-image-prelaoder') );?><?php
} else {
...
<form method="post" action="">
	<table class="form-table">
		...
		<!-- Sidebar ABout Box-->
		<tr>
			<th colspan="2"><strong>Header Ad 469x60</strong></th>
		</tr>
		<tr>
			<th><label for="ads1">Ads Section Enabled:</label></th>
			<td>
				<select name="ads1" id="ads1">
					<option value="no" <?php if(get_option('cici_ads1') == 'no'){?>selected="selected"<?php }?>>No</option>
					<option value="yes" <?php if(get_option('cici_ads1') == 'yes'){?>selected="selected"<?php }?>>Yes</option>
				</select> 
			</td>
		</tr>
		<tr valign="top">
			<th scope="row"><label for="image_url5">Image URL 1</label></th>
			<td>
				<input name="image_url5" type="text" id="image_url5" value="<?php echo get_option('cici_image_url5'); ?>" class="regular-text" />
			</td>
		</tr>
		
		<tr valign="top">
			<th scope="row"><label for="banner_url5">Banner URL 1</label></th>
			<td>
				<input name="banner_url5" type="text" id="banner_url5" value="<?php echo get_option('cici_banner_url5'); ?>" class="regular-text" />
			</td>
		</tr>
		<tr>
